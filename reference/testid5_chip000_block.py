'''
blockstatus, terase, blockbe, tread, tprog write into mysql
Author: Hu Tao
Dependencies: pymsql datetime
Usage: python3 nandflash_to_mysql.py
'''
    ...
    with open(file_path) as f:
        ...
        while line:
            ...
            if i>=start_lines:
                ...
                '''
                向tread表中插入数据
                从blockstatus数据表中提取pe和health值，health值判断block有没有坏，pe作为该block下read的pe次数，并且计算blockbe表中block_err:取2304个16K页中err最高的前115个err的平均值
                '''
                if line=="[Group PEcnt CE LUN Block Page  Type  ][ tR ][page_err][F[0]   F[1]   F[2]   F[3]   F[4]   F[5]   F[6]   F[7]   F[8]   F[9]   F[a]   F[b]   F[c]   F[d]   F[e]   F[f]   ]\n":
                    block_err=[]
                    k=0
                ...
                if len(x1) ==7 and x1[0]!="Group" and len(x)==4:
                    ...
                    blockID = str(chip).zfill(3) + str(x1[2][1:].zfill(2)) + str(x1[3][1:].zfill(2)) + str(x1[4][1:].zfill(4))
                    ...
                    if results[0][1] == 0:
                        ...
                        '''
                        sql = "INSERT INTO tread(testID,pe,rt,rd,chip,ce,die,block,page,pagetype,tread,err,f0,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,recordtime) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"

                        cursor = db.cursor()

                        
                        try:
                            cursor.execute(sql,(read_record[1],results[0][0],read_record[3],read_record[4],read_record[5],read_record[6],
                                                read_record[7],read_record[8],read_record[9],read_record[10],read_record[11],read_record[12],
...
if __name__=="__main__":
    ...
    db = pymysql.connect(host='127.0.0.1', port=3306,
                         user='root', passwd='nvmgroup', db='nandflash', charset='utf8')
    ...
    for item in chip:
        ...
        chip=item
        read_file(file_path,chip,db,testID,start_lines,exit_lines)
